...
uses
  ...
  rpsection,rpprintitem,rpdatainfo;
...
function TUndoCue.GetObjectByName(const name: string): TObject;
var
 idx: integer;
 compo: TObject;
begin
...
var
  ...
  nvalue: Variant;
begin
  target := GetObjectByName(operation.componentName);
  if target = nil then
    Exit;
  ...
  begin
    ...
    oldParentSection := TRpSection(GetObjectByName(oldParentName));
    newParentSection := TRpSection(GetObjectByName(newParentName));
